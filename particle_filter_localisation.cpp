...
void ParticleFilter::initialiseParticles()
{
  ...
  // You probably need to use a "." in your numbers (e.g. "1.0") when calculating the weights
	...
	{
		//x direction
		particles_[i].x = randomUniform(map_x_min_ , map_x_max_);

		//y direction
		particles_[i].y = randomUniform(map_y_min_ , map_y_max_);
	
		//theta angle
		particles_[i].theta = randomUniform(0, M_PI*2);

		//weightage of the particles
...
void ParticleFilter::normaliseWeights()
{
  // Normalise the weights of the particles in "particles_"
	//sum all values in wieghtage and divide by sum
  ...
  // YOUR CODE HERE //
	...
	for (int i =0 ; i < (num_particles_ + 1) ; i++)
	{
		particles_[i].weight /= normaliser;
	}
...
void ParticleFilter::estimatePose()
{
	...
	//variables to hold the sum of all weighted sin and cos values 
	double sum_sin ;
	double sum_cos ;	
 
	for (int i=0 ; i < (num_particles_ +1)  ; i++)
	{
	estimated_pose_x += particles_[i].x *   particles_[i].weight ; 
	estimated_pose_y += particles_[i].y *   particles_[i].weight ; 

	//the sums used to find average theta
	sum_sin += std::sin((particles_[i].theta)) *  particles_[i].weight;
	sum_cos += std::cos((particles_[i].theta)) *  particles_[i].weight;
	
	}
	
	estimated_pose_theta = std::atan2( (sum_sin / particles_.size()),(sum_cos / particles_.size() ) );
...
void ParticleFilter::odomCallback(const nav_msgs::Odometry& odom_msg)
{
  ...
  // YOUR CODE HERE
	...
	//variable declaration for distance and rotation noise
	double w_d ; 
	double w_theta;
	...
	//assign random normal value for the 2 noises
	w_d = randomNormal(motion_distance_noise_stddev_);
	w_theta = randomNormal(motion_rotation_noise_stddev_);

	//Update position in x orientation
	particles_[i].x = particles_[i].x + ((distance + w_d) * wrapAngle(std::cos(theta))) ;

	//Update position in y orientation
	particles_[i].y = particles_[i].y + ((distance + w_d) * wrapAngle(std::sin(theta)));

	//Update position in theta orientation
	particles_[i].theta = wrapAngle((particles_[i].theta + rotation) + w_theta);
...
void ParticleFilter::scanCallback(const sensor_msgs::LaserScan& scan_msg)
{
  ...
  for (auto& p : particles_)
  {
	...
	for (int i = 0; i < scan_msg.ranges.size(); i += step)
	{
		...
		// find likelihood of getting measured
		measured_likelihood = (1/std::sqrt(2* M_PI * std::pow(sensing_noise_stddev_ , 2)))  * std::exp(- (std::pow((particle_range - scan_range) , 2 ))/(2* std::pow(sensing_noise_stddev_ , 2))) ; 

		// likelihood for all measured beams 
		likelihood *= measured_likelihood;
